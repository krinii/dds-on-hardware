...
static TestDataTyping_data data[MAX_SAMPLES];
...
int main (int argc, char ** argv)
{
  ...
  topic = dds_create_topic (
    participant, &TestDataTyping_data_desc, "TestDataType_data", NULL, NULL);
  if (topic < 0)
  ...
  {
    TestDataTyping_data_free (&data[i], DDS_FREE_CONTENTS);
  }
...
void data_available(dds_entity_t reader, void *arg){
  ...
  for (int i = 0; i < rc; i ++){
    ...
    //if ((rc > 0) && (infos[i].valid_data) && (infos[i].sample_state == DDS_SST_NOT_READ)){
      /* Print Message. */
      msg = (TestDataTyping_data*) samples[i];
      sample_Count++;
      //printf ("%d, ", msg->msgNr);
