...
int main (int argc, char ** argv)
{
  ...
  dds_listener_t *listenerTopic = NULL;
  ...
  listenerTopic = dds_create_listener(NULL);
  dds_lset_inconsistent_topic(listenerTopic, inconsistent_topic);
  ...
  topic = dds_create_topic (
    participant, &TestDataType_data_desc, "TestDataType_data", NULL, listenerTopic);
  if (topic < 0)
...
void data_available(dds_entity_t reader, void *arg){
  ...
  for (int i = 0; i < rc; i ++){
    ...
    //if ((rc > 0) && (infos[i].valid_data) && (infos[i].sample_state == DDS_SST_NOT_READ)){
      ...
      sample_Count++;
      //printf ("%d, ", msg->msgNr);
      printf ("Value: %d, arr[0]: %.4f, %d\n", msg->value, msg->arr[0], sample_Count);
    }
  }
