...
#define DEPTH 100 // Irrelevant with history kind HISTORY_KEEP_ALL
...
TestDataType_Speed_data *msg;
static TestDataType_Speed_data data[MAX_SAMPLES];
...
int main (int argc, char ** argv)
{
  ...
  topic = dds_create_topic (
    participant, &TestDataType_Speed_data_desc, "TestDataType_Speed_data", NULL, NULL);
  if (topic < 0)
  ...
  {
    TestDataType_Speed_data_free (&data[i], DDS_FREE_CONTENTS);
  }
