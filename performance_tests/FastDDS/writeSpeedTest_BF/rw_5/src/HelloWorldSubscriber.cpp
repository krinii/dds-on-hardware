            ...
            if (reader->take_next_sample(&hello_, &info) == ReturnCode_t::RETCODE_OK)
            {
                if ((info.instance_state == ALIVE) && (info.valid_data))
                {
                    ...
                    /*if (DEBUG){
                        ...
                        std::array<double, 1> val = hello_.arr();
...
public:
    ...
    {
        FILE *fp;
        fp = fopen("/home/k/Desktop/Workspaces/Master_Project/Git_Repositories/dds-on-hardware/performance_tests/FastDDS/writeSpeedTest_BF/rw_5/data/test_5.txt", "a");
        fprintf(fp, "NewRound\n");
        fclose(fp);

        int tmp_msg_count = 0;
        
        for (int i = 0; (i < 101) && (sigintH); i++){
            listener_.samples_ = 0; // Reset counter
            ...
            std::cout << "Sample count: " << tmp_msg_count << std::endl;
            
        }
        /*while((listener_.samples_ < samples) && sigintH)
        {
            std::this_thread::sleep_for(std::chrono::milliseconds(5000));
        }*/
    }
        ...
        char** argv)
{
    ...
    std::cout << "Subscriber cleanup" << std::endl;
    delete mysub;
    return 0;
}
